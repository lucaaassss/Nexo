...
export function ProjectChat() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-10rem)] max-w-6xl mx-auto">
      ...
      <div className="flex-1 bg-zinc-950/60 border border-zinc-800/80 rounded-2xl flex flex-col justify-between overflow-hidden shadow-xl">
        {/* Header del Chat */}
        <div className="p-4 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-950/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400">Toda la comunicación centralizada</p>
            </div>
          </div>
          ...
          <div className="relative w-48 sm:w-64">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl pl-8 pr-3 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-violet-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {filteredMessages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-zinc-500 text-xs py-12">
              ...
              <p>El canal está listo. Sé el primero en enviar un mensaje.</p>
            </div>
          ) : (
            filteredMessages.map((msg) => {
              ...
              return (
                ...
                >
                  {/* Avatar */}
                  <div className="w-7 h-7 rounded-full bg-violet-600/30 text-violet-300 font-bold text-[10px] flex items-center justify-center border border-violet-500/30 shrink-0">
                  ...
                  <div className={`max-w-md space-y-1 ${isMine ? 'text-right' : 'text-left'}`}>
                    ...
                    {parentMsg && (
                      <div className="text-[10px] p-2 rounded-lg bg-zinc-900/80 border border-zinc-800 text-zinc-400 italic mb-1">
                    ...
                    {/* Burbuja del Mensaje */}
                    <div
                      className={`p-3 rounded-2xl text-xs leading-relaxed inline-block shadow-md ${
                        isMine
                      ...
                      <div className="flex flex-wrap gap-1 pt-1">
                        {msg.reactions.map((r) => (
                          <button
                            ...
                            className="px-1.5 py-0.5 rounded-full bg-zinc-900 border border-zinc-800 text-[10px] flex items-center gap-1 hover:border-violet-500/40"
                          >
                            <span>{r.emoji}</span>
                          </button>
                        ))}
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-[10px] pt-0.5">
                      ...
                      <button
                        ...
                        className="text-zinc-500 hover:text-violet-400 ml-2"
                      >
        ...
        {isTyping && (
          <div className="px-4 py-1 text-[10px] text-violet-400 italic animate-pulse">
        ...
        {replyingTo && (
          <div className="px-4 py-2 bg-zinc-900 border-t border-zinc-800 flex items-center justify-between text-xs text-zinc-400">
        ...
        {/* Entrada de Mensaje */}
        <form onSubmit={handleSendMessage} className="p-3 border-t border-zinc-800 bg-zinc-950/80 flex items-center gap-2">
          <label className="p-2 text-zinc-400 hover:text-white rounded-xl hover:bg-zinc-900 cursor-pointer transition-colors">
          ...
          <input
            ...
            className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-3.5 py-2 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-violet-500"
          />

          <button
            type="submit"
            className="p-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-600/30 transition-all"
          >
      ...
      <div className="w-full lg:w-64 bg-zinc-950/60 border border-zinc-800/80 rounded-2xl p-4 space-y-4">
        ...
        <div className="space-y-2">
          {(currentProject?.members || [
            ...
            <div key={m.id} className="flex items-center justify-between p-2 rounded-xl bg-zinc-900/40 border border-zinc-800/60">
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <span className="w-2 h-2 rounded-full bg-emerald-500 absolute -bottom-0.5 -right-0.5 ring-2 ring-zinc-950" />
                </div>
                <span className="text-xs text-zinc-200 font-medium truncate max-w-[100px]">
                  {m.user?.name}
                </span>
              </div>
