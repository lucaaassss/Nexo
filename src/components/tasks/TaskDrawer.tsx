  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-2xl bg-zinc-900 border-l border-zinc-800 shadow-2xl flex flex-col justify-between">
          {/* Header del Panel */}
          <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/60">
            <div className="flex items-center gap-3">
              <span className="font-mono text-xs font-bold text-violet-400 bg-violet-950/40 border border-violet-500/30 px-2.5 py-1 rounded-lg">
                {task.key}
              </span>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
              <input
                ...
                className="w-full text-xl font-bold text-zinc-100 bg-transparent border-b border-transparent hover:border-zinc-800 focus:border-violet-500 focus:outline-none py-1 transition-colors"
              />
            </div>

            {/* Selectores de Estado y Prioridad */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 bg-zinc-950/50 rounded-2xl border border-zinc-800/80">
              <div>
            ...
            <div className="p-4 bg-zinc-950/60 rounded-2xl border border-zinc-800/80 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-zinc-200">Seguimiento de Tiempo</span>
                </div>
                <div className="text-xs font-mono text-zinc-400">
                  <span className="text-violet-400 font-bold">{task.loggedHours || 0}h</span> registrados de{' '}
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                ...
                {!isTimerRunning ? (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 text-xs font-semibold hover:bg-emerald-600/30 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-600/20 text-rose-400 border border-rose-500/30 text-xs font-semibold hover:bg-rose-600/30 transition-colors animate-pulse"
                  >
                ...
                <div className="flex items-center gap-1.5 ml-auto">
                  <input
                    ...
                    className="w-20 bg-zinc-900 border border-zinc-800 rounded-xl px-2.5 py-1 text-xs text-zinc-200 focus:outline-none focus:border-violet-500"
                  />
                  <button
                    onClick={handleAddManualTime}
                    className="px-2.5 py-1 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-medium transition-colors"
                  >
                    Añadir
                  </button>
                </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-zinc-950/60 border border-zinc-800 rounded-2xl p-3.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-violet-500 resize-none leading-relaxed"
              />
            </div>
              ...
              <div className="space-y-2">
                {(task.subtasks || []).map((st) => (
                  <div
                    ...
                    className="flex items-center gap-2.5 p-2.5 rounded-xl bg-zinc-950/40 border border-zinc-800/60 hover:bg-zinc-800/40 cursor-pointer transition-colors"
                  >
                    {st.completed ? (
                ...
                <form onSubmit={handleAddSubtaskSubmit} className="flex gap-2 pt-1">
                  <input
                    ...
                    className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-violet-500"
                  />
                  <button
                    type="submit"
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <label className="cursor-pointer text-xs text-violet-400 hover:text-violet-300 font-medium flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {task.attachments?.map((att) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="p-2.5 rounded-xl bg-zinc-950/60 border border-zinc-800 hover:border-violet-500/30 flex items-center gap-2 group transition-colors"
                    >
                      <FileText className="w-4 h-4 text-violet-400 shrink-0" />
                      <div className="truncate text-left">
                        <p className="text-xs font-medium text-zinc-200 truncate group-hover:text-violet-300">
                          {att.name}
                        </p>
            ...
            <div className="space-y-4 pt-4 border-t border-zinc-800">
              ...
              <div className="space-y-3">
                {(task.comments || []).map((cmt) => (
                  <div key={cmt.id} className="p-3 rounded-2xl bg-zinc-950/60 border border-zinc-800/80 space-y-1">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-zinc-400 leading-relaxed">{cmt.content}</p>
                  </div>
                ))}
              </div>
              ...
              <form onSubmit={handleAddCommentSubmit} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-3.5 py-2 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-violet-500"
                />
                <button
                  type="submit"
