      ...
      <div className="flex gap-4 overflow-x-auto pb-6 pt-2 items-start min-h-[calc(100vh-12rem)]">
        {KANBAN_COLUMNS.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className="w-72 sm:w-80 shrink-0 bg-zinc-950/40 border border-zinc-800/80 rounded-2xl flex flex-col max-h-[calc(100vh-14rem)]"
            >
              {/* Encabezado de la Columna */}
              <div className="p-3.5 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-950/60 rounded-t-2xl">
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-full ${colStatus.dot}`} />
                  <h3 className="text-xs font-bold text-zinc-200 uppercase tracking-wider">
                    {col.title}
                  </h3>
                  <span className="text-[11px] font-mono font-semibold px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400">
                    {colTasks.length}
                  </span>
                </div>
                <button
                        ...
                        <Draggable key={task.id} draggableId={task.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`p-4 rounded-2xl bg-zinc-900 border border-zinc-800 hover:border-violet-500/40 transition-all cursor-pointer shadow-md group ${
                                snapshot.isDragging ? 'rotate-2 scale-105 shadow-2xl ring-2 ring-violet-500 z-50' : ''
                              }`}
                            >
                              {/* Clave y Prioridad */}
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-[10px] font-mono font-semibold text-zinc-500 group-hover:text-violet-400 transition-colors">
                                  {task.key}
                                </span>
                              ...
                              {/* Título de Tarea */}
                              <h4 className="text-xs font-semibold text-zinc-100 leading-snug mb-3">
                                {task.title}
                              </h4>
