    ...
    <div className="space-y-4 max-w-5xl mx-auto pb-10">
      {STATUS_GROUPS.map((group) => {
        ...
        return (
          <div
            key={group.id}
            className="bg-white dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-sm"
          >
            {/* Header del Grupo */}
            <div
              onClick={() => toggleGroup(group.id)}
              className="p-3.5 flex items-center justify-between bg-zinc-100/80 dark:bg-zinc-950/80 cursor-pointer hover:bg-zinc-200/60 dark:hover:bg-zinc-900/60 transition-colors"
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded-full bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-400">
                  {groupTasks.length}
                </span>
              </div>
              {onOpenNewTask && (
                  ...
                  groupTasks.map((task) => {
                    ...
                    return (
                      <div
                        ...
                        className="p-3 sm:px-4 flex items-center justify-between gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/60 transition-colors cursor-pointer group"
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <span className="text-xs font-mono font-bold text-zinc-500 dark:text-zinc-500 group-hover:text-violet-600 dark:group-hover:text-violet-400 w-16 shrink-0">
                            {task.key}
                          </span>
                          <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 truncate flex-1">
                            {task.title}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          {task.dueDate && (
                            <span className="text-[11px] font-medium text-zinc-500 dark:text-zinc-500 flex items-center gap-1 hidden sm:flex">
